classdef User < Module
    ...
    methods
        function obj = User(p, index)
            ...
            if p.user.random_drop
                distance_range = p.user.distance_range(2) - p.user.distance_range(1);
                obj.distance = users.distance_range(1) + rand*distance_range;
                
                theta_range = p.user.theta_range(2) - p.user.theta_range(1);
                obj.angle = users.theta_range(1) + rand*theta_range;
            else
                obj.distance = p.user.distance(index);
                obj.angle = p.user.theta(index);
            end
        end
